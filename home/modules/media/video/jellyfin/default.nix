...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.dataFile."jellyfinmediaplayer/mpv.conf" = lib.mkIf config.pbor.media.video.mpv.enable {
      source = config.lib.file.mkOutOfStoreSymlink "${config.xdg.configHome}/mpv/mpv.conf";
    };

    wayland.windowManager.sway.config.window.commands = lib.mkIf config.pbor.wm.sway.enable [
      {
